...
train_x,test_x,train_y,test_y = train_test_split(signal, labs, test_size=0.05,
...
def cnn_layer():
    ...
    with tf.name_scope('Conv_1'):
        ...
        pool1 = max_pool_1d(conv1, nn_config["nn_para1"]["W_p"])
    ...
    with tf.name_scope('Conv_2'):
        ...
        pool2 = max_pool_1d(conv2, nn_config["nn_para2"]["W_p"])
    ...
    with tf.name_scope('Conv_3'):
        ...
        pool3 = max_pool_1d(conv3, nn_config["nn_para3"]["W_p"])
    ...
    with tf.name_scope('Conv_4'):
        ...
        pool4 = max_pool_1d(conv4, nn_config["nn_para4"]["W_p"])
        drop4 = dropout(pool4, keep_prob_5)

    with tf.name_scope('Conv_5'):
        weight5 = weight_variable(nn_config["nn_para5"]["W_k"])
        bias5 = bias_variable([nn_config["nn_para5"]["W_k"][-1]])
        conv5 = prelu(conv2d(drop4, weight5, nn_config["nn_para5"]["W_s"]) + bias5)
        pool5 = max_pool_1d(conv5, nn_config["nn_para5"]["W_p"])
        drop5 = dropout(pool5, keep_prob_5)

    with tf.name_scope('Conv_6'):
        weight6 = weight_variable(nn_config["nn_para6"]["W_k"])
        bias6 = bias_variable([nn_config["nn_para6"]["W_k"][-1]])
        conv6 = prelu(conv2d(drop5, weight6, nn_config["nn_para6"]["W_s"]) + bias6)
        pool6 = max_pool_1d(conv6, nn_config["nn_para6"]["W_p"])
        drop6 = dropout(pool6, keep_prob_5)

    # 全连接层
    with tf.name_scope('FC_1'):
        to_fc_size = drop6.get_shape().as_list()[-2:]
        drop7_flat = tf.reshape(drop6, [-1, to_fc_size[0]*to_fc_size[1]])
        Wf1 = weight_variable([to_fc_size[0] * nn_config["nn_para6"]["W_k"][-1], nn_config["fc_para1"]["W_1"]])
        bf1 = bias_variable([nn_config["fc_para1"]["W_1"]])
        dense1 = prelu(tf.matmul(drop7_flat, Wf1) + bf1)
...
def cnn_train():
    ...
    with tf.Session() as sess:
        ...
        # next_global_step = tf.assign(written_step,written_step+1)
            ...
            while True:
                for i in range(num_batch):
                    ...
                    if global_step % 100 == 0:
                        ...
                        print("{:}  train1 accuracy rate is {:.4},and loss is {:}".format(global_step, acc1,loss1))
                        print("{:}  train2 accuracy rate is {:.4},and loss is {:}".format(global_step, acc2,loss2))
                        print("Cost time is {}s".format(time.time() - time_start))
        ...
        print("Totally cost time is {:.2}s".format(time.time() - time_start))



cnn_train()
# todo:加载已有模型再训练，划分为三个训练集
