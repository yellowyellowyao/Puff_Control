...
# import for_network3 as nn3
from sklearn.model_selection import train_test_split
import random
import numpy as np
import time
import config_of_training as tc
from functools import partial

nn_config = choice_network_config(3)
...
def cnn_layer_for_single_channel(nn_config, inp, i):
    # 第一层卷积
    with tf.name_scope(tc.training_channels[i]+'_Conv_1'):
        weight1 = weight_variable(nn_config["nn_para1"]["W_k"])
        bias1 = bias_variable([nn_config["nn_para1"]["W_k"][-1]])
        conv1 = prelu(conv1d(inp, weight1, nn_config["nn_para1"]["W_s"]) + bias1)
        pool1 = max_pool_1d(conv1, nn_config["nn_para1"]["W_p"])
        drop1 = dropout(pool1, keep_prob_5)

    with tf.name_scope(tc.training_channels[i]+'_Conv_2'):
        weight2 = weight_variable(nn_config["nn_para2"]["W_k"])
        bias2 = bias_variable([nn_config["nn_para2"]["W_k"][-1]])
        conv2 = prelu(conv1d(drop1, weight2, nn_config["nn_para2"]["W_s"]) + bias2)
        pool2 = max_pool_1d(conv2, nn_config["nn_para2"]["W_p"])
        drop2 = dropout(pool2, keep_prob_5)

    with tf.name_scope(tc.training_channels[i]+'_Conv_3'):
        weight3 = weight_variable(nn_config["nn_para3"]["W_k"])
        bias3 = bias_variable([nn_config["nn_para3"]["W_k"][-1]])
        conv3 = prelu(conv1d(drop2, weight3, nn_config["nn_para3"]["W_s"]) + bias3)
...
def cnn_layer(nn_config):
    layers_tower = []
    for i in range(tc.Channels_nums):
        # inp = tf.reshape(x[:, i, :, :], [-1, Channels_len[training_channels[i]], 1])
        inp = tf.reshape(x[:, i, :, :], [-1, Signal_max_len, 1])
        layers_tower.append(cnn_layer_for_single_channel(nn_config, inp, i))

    with tf.name_scope('Conv_4'):
        conv4_input = tf.stack(layers_tower,axis=1)
        weight4 = weight_variable(nn_config["nn_para4"]["W_k"])
        bias4 = bias_variable([nn_config["nn_para4"]["W_k"][-1]])
        conv4 = prelu(conv2d(conv4_input, weight4, nn_config["nn_para4"]["W_s"]) + bias4)
